...
function combinedFilter() {
  ...
  if (selectedYear === 'all' && selectedGenre === 'all' && selectedRating === 'all') {
    // Show all containers if all "All" options are selected
    aniContainerElements.forEach((element) => {
      element.style.display = 'block';
    });
  } else {
    ...
    // Show containers that match all filter criteria
    ...
    aniContainerElements.forEach((element) => {
      ...
      const ratingElement = element.querySelector('.aniRating.name .value');
      const ratingValue = parseFloat(ratingElement.textContent);

      const [min, max] = selectedRating.split(' to ').map(Number);

      if (
        (year === selectedYear || selectedYear === 'all') &&
        (genre.includes(selectedGenre.toLowerCase()) || selectedGenre === 'all') &&
        (ratingValue >= min && ratingValue <= max || selectedRating === 'all')
      ) {
        element.style.display = 'block';
        matchFound = true;
      }
    ...
    if (!matchFound) {
      ...
      noResultsMessage.textContent = 'No results found for the selected genre, year, and rating.';
    } else {
      const noResultsMessage = document.getElementById('no-results-message');
