...
function createYouTubePlayer(videoId, movieId) {
  var container = document.getElementById(movieId);
  if (!container) {
    console.error(`Container with ID ${movieId} not found.`);
    return;
  }
      ...
      link.addEventListener('click', function(event) {
        ...
        if (url.includes('youtube.com/watch') || url.includes('youtu.be/')) {
          ...
          if (videoId) {
            ...
            var movieDiv = link.closest('.aniContainer');
            if (movieDiv && movieDiv.id) {
              createYouTubePlayer(videoId, movieDiv.id);
            } else {
              console.error("Could not find the movie container ID.");
            }
          } else {
