(function() {
  ...
  const movieElements = Array.from(parentElement.querySelectorAll('div[id^="movie_"]'));

  const elementsWithName = [];

  for (const element of movieElements) {
    const nameElement = element.querySelector('.aniName .value');
    const idMatch = element.id.match(/^movie_(\d+)$/);
  ...
  for (let i = 0; i < elementsWithName.length; i++) {
    ...
    sortedElement.id = `movie_${newIdNumber}`;
  ...
  console.log(`Successfully sorted and reordered ${elementsWithName.length} movie elements on page load.`);
})();